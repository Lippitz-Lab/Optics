...
\begin{tikzpicture}[BS style/.style={draw=gray, line width=3pt},
    ...
    \coordinate (m3) at (0,1);
    \coordinate (m2) at (1,1);
    \coordinate (in) at (-1,0);
    \coordinate (out) at (0, -1);

    \draw[thick, -> ] (in) -- (m1) -- (m2) -- (m3) -- (out) node[right] {$E_{out}$};
    \draw[thick, -> ] (in) -- ++(0:0.5) node[below] {$E_{in}$};

    \draw[style=BS style] (BS) ++ (45:\len) -- ++(-135:{2*\len});
    \draw[style=mirror style] (m1) ++ (45:\len) -- ++(-135:{2*\len});
    \draw[style=mirror style] (m2) ++ (135:\len) -- ++(-45:{2*\len});
    \draw[style=mirror style] (m3) ++ (45:\len) -- ++(-135:{2*\len});
